...
int LinuxErrorsDumper::ErrorHandler(void* d, void* evt) {
  ...
  char mesg[BUFSIZ];
  char number[32];
  GLOG_DEBUG("Current DISPLAY %s; XAUTHORITY %s", getenv("DISPLAY"),
             getenv("XAUTHORITY"));
  ...
  if ((event->error_code == BadWindow) || (event->error_code == BadPixmap) ||
      (event->error_code == BadCursor) || (event->error_code == BadFont) ||
      (event->error_code == BadDrawable) || (event->error_code == BadColor) ||
      (event->error_code == BadGC) || (event->error_code == BadIDChoice) ||
    ...
    if (event->error_code == BadValue)
      ...
      XGetErrorDatabaseText(dpy, mtype, "ResourceID", "ResourceID 0x%x", mesg,
                            BUFSIZ);
  ...
  XGetErrorDatabaseText(dpy, mtype, "CurrentSerial", "Current Serial #%d", mesg,
                        BUFSIZ);
  GLOG_DEBUG(mesg, -1);
  //////////////////
  return 0;
