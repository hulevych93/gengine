...
struct WindowsCommunicationEngine::CallingContext: public boost::static_visitor<void>
{
...
void WindowsCommunicationEngine::RegisterConnection(const IChannel& connection, engine_callback callback)
{
    ...
    else
    {
        ...
        m_clientCallbacks.emplace(std::make_pair(handle, std::make_unique<CallingContext>(const_cast<NamedPipeChannel&>(pipeChannel), callback)));
    }
}

void WindowsCommunicationEngine::UnregisterConnection(const IChannel& connection)
{
    auto& pipeChannel = static_cast<const NamedPipeChannel&>(connection);
    auto handle = pipeChannel.GetIOHandle();
...
{
    auto waitStatus = Multithreading::Event::WaitForEventsEx(&m_eventPool[0], m_eventPool.size());
    switch (waitStatus)
    {
    case WAIT_OBJECT_0:
        break;
