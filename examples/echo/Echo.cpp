...
class EchoServer : public Echo::IEcho, Worker {
 public:
  EchoServer(const char* client, const char* server)
      : Worker(200), m_echoClient(client), m_echoServer(server, *this) {}

  void init() { m_echoServer.Reveal(); }

  void deinit() { m_echoServer.Hide(); }
...
class TestEcho : public Entries::EntryBase, public Echo::IEcho {
 public:
