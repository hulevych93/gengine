...
public:
    ...
    template<class T>
    typename std::enable_if<std::is_enum<T>::value, bool>::type operator<<(const T& data)
    {
    ...
    {
        bool result = operator<<(object != nullptr ? static_cast<std::int8_t>(1) : static_cast<std::int8_t>(0));
        if(object)
    ...
    {
        bool result = operator<<(object ? static_cast<std::int8_t>(1) : static_cast<std::int8_t>(0));
        if (object)
