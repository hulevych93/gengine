...
void TCPSocket::AsyncConnect(const std::string & address, std::uint16_t port, connect_cb cb)
{
    boost::system::error_code error;
    boost::asio::ip::tcp::resolver::query query(address, boost::lexical_cast<std::string>(port));
    boost::asio::ip::tcp::resolver::iterator endpoint_iterator = m_resolver.resolve(query, error);
    if (!error)
    {
        boost::asio::async_connect(m_socket, endpoint_iterator, [cb](const boost::system::error_code& ec, boost::asio::ip::tcp::resolver::iterator it) {
            cb(ec);
        });
    }
    else
