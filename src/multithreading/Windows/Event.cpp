...
std::uint32_t Event::WaitForEventsEx(void* events, std::uint32_t count)
{
    ...
    if (count <= MAXIMUM_WAIT_OBJECTS)
    {
        ...
        waitResult = WaitForMultipleObjects(count, reinterpret_cast<HANDLE*>(events), FALSE, INFINITE);
    }
    else
    {
        ...
        // in this case, all threads should be finished as soon as one event will set
        ...
        while (currentIndex < count)
        {
            ...
            {
                eventGroup[countInGroup++] = reinterpret_cast<HANDLE*>(events)[currentIndex++];
            }

            waitGroups.push_back(eventGroup);

            auto handler = [countInGroup, eventGroup, offset, &waitResult, &waitResultEvent]() {
                DWORD result = WaitForMultipleObjects(countInGroup, reinterpret_cast<HANDLE*>(eventGroup), FALSE, INFINITE);
        ...
        std::for_each(threadsGroup.begin(), threadsGroup.end(), std::bind(&std::thread::join, std::placeholders::_1));
