...
void WorkerThread::Initialize()
{
    if (!m_executorThread)
    {
        m_executorThread = std::make_unique<std::thread>(std::bind(&WorkerThread::ThreadProc, this));
...
void WorkerThread::Dispose()
{
    ...
    if (m_executorThread->joinable() && std::this_thread::get_id() != m_executorThread->get_id())
    {
...
std::int32_t WorkerThread::StartTimer(task_t task, std::uint32_t intervalMS, std::uint32_t firstDelayMS /*= 0*/)
{
    if (!m_canRun)
    {
        return - 1;
    ...
    while (m_canRun)
    {
        ...
        for (const auto& localTimer : localTimers)
        {
            if (localTimer.second->nextScheduledTime <= now)
            {
                ...
                now = getTimeInMills();
                localTimer.second->nextScheduledTime = now + localTimer.second->intervalMS;
