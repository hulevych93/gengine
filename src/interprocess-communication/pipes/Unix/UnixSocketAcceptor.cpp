...
UnixSocketAcceptor::UnixSocketAcceptor(const std::wstring& socketFileName, const std::shared_ptr<CommunicationEngine>& engine)
        ...
        if (listeningHandle  != InvalidHandle)
        {
            ...
            if (nRet != -1)
            {
                ...
                if(bind(listeningHandle , (sockaddr*)&addr, SUN_LEN(&addr)) != -1)
                {
                    ...
                    {
                        m_channel = std::make_unique<UnixDomainChannel>(listeningHandle, std::static_pointer_cast<UnixSocketEngine>(m_engine));
                            ...
                            if(acceptedSocket != InvalidHandle)
                            {
                                if (fcntl(acceptedSocket, F_SETFL, fcntl(acceptedSocket, F_GETFL) | O_NONBLOCK) != -1)
                                {
                                    ...
                                    if (::setsockopt(acceptedSocket, SOL_SOCKET, SO_LINGER,
                                                     (char *)& lingerStruct, sizeof(lingerStruct)) != -1)
                                    {
                                        callback(true, std::make_unique<UnixDomainChannel>(acceptedSocket,
