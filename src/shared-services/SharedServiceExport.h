...
#define EXPORT_CREATOR_SHARED_SERVICE(implementation)           \
  ...
  BOOST_DLL_ALIAS(Create##implementation##Service,              \
                  create_##implementation##_service)

// todo

#define EXPORT_SHARED_ENTRY_POINT(implementation)                   \
  static boost::shared_ptr<IEntry> Create##implementation##Service( \
      std::unique_ptr<IEntryToolsFactory>&& factory) {              \
    return boost::shared_ptr<implementation>(                       \
        new implementation(std::move(factory)));                    \
  }                                                                 \
  BOOST_DLL_ALIAS(Create##implementation##Service, create_shared_entry)

#if __WIN32__
#define IMPLEMENT_SHARED_ENTRY                                       \
  void* g_module_instance = nullptr;                                 \
  BOOL APIENTRY DllMain(HMODULE ModuleInstance, DWORD ReasonForCall, \
                        LPVOID Reserved) {                           \
