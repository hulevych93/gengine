...
bool PatcherModule::PatcherBase::Register(void* options)
{
    ...
    programOptions("file", boost::program_options::wvalue<std::wstring>(&m_filePath), "file")
        ("config", boost::program_options::wvalue<std::wstring>(&m_configPath), "config");
    return true;
}
...
bool PatcherModule::PatcherBase::Process(void*, bool* success)
{
    assert(success);
    *success = false;
    if (Filesystem::IsFileExists(m_filePath) && Filesystem::IsFileExists(m_configPath))
    {
        ...
        if (!configBuff.empty())
        {
            ...
            if (file.is_open())
            {
                auto fileName = Filesystem::GetFileNameWithoutExtension(fs::path(m_filePath).filename().wstring());
                auto begin = boost::str(boost::format(ConfigDelemiterStart) % toUtf8(fileName));
                auto end = boost::str(boost::format(ConfigDelemiterEnd) % toUtf8(fileName));
...
PatcherModule::EntryPatcher::EntryPatcher()
    : PatcherBase(L"executable")
{}
...
PatcherModule::PluginPatcher::PluginPatcher()
    : PatcherBase(L"plugin")
{}
...
bool PatcherModule::CreateProcessors(std::vector<std::unique_ptr<ICmdProcessor>>* processors)
{
    assert(processors);
