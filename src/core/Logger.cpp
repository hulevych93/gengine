...
void Logger::Log(const char* function_name, std::size_t line, type_t type, level_t level, const char* fmt, va_list args) const
{
    try
    {
...
void Logger::Log(const char* function_name, std::size_t line, type_t type, level_t level, boost::format& format) const
{
    try
    {
        if (GetLoggerProxy()->IsAvailable(type))
        {
            ...
            {
                prefix = boost::str(boost::format("[%1%:%2%]: ") % function_name % line);
            }
