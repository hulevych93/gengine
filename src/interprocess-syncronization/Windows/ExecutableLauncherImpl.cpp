...
void ExecutableLauncherImpl::AddExecutable(const executable_params& params, IExecutableLauncherListener& listener)
{
    m_ChangeEvent.Set();
...
bool ExecutableLauncherImpl::OnSessionChange(std::uint32_t event, std::uint32_t sessionId)
{
   return true;
}

void ExecutableLauncherImpl::OnExecutableLaunched(const std::shared_ptr<Executable>& app)
{
...
void ExecutableLauncherImpl::OnExecutableClosed(const std::shared_ptr<Executable>& app)
{
    auto handleIter = std::find(m_eventPool.begin(), m_eventPool.end(), app->GetHandle());
