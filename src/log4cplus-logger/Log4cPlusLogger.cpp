...
static const std::string PatternLayout = "%d{%d-%m-%y %H:%M:%S.%q} [%c{2}:%t] [%-5p] %m %n";
...
public:
        ...
        : m_name()
    {
...
#ifdef BUILD_WINDOWS
        {
            ...
            win32debugAppender->setLayout(std::auto_ptr<log4cplus::Layout>(new log4cplus::PatternLayout(LOG4CPLUS_C_STR_TO_TSTRING(PatternLayout))));
...
protected:
    ...
    void SetConsoleOutput(bool use_stdout)
    {
        ...
        auto consoleAppender = root.getAppender(LOG4CPLUS_C_STR_TO_TSTRING(Console));
        if (consoleAppender)
        {
            if (!use_stdout)
            {
        ...
        else
        {
            if (use_stdout)
            {
                ...
                consoleAppender->setLayout(std::auto_ptr<log4cplus::Layout>(new log4cplus::PatternLayout(LOG4CPLUS_C_STR_TO_TSTRING(PatternLayout))));
    ...
    void SetFileRolling(const std::string& filename)
    {
        ...
        if (!filename.empty())
        {
            auto fileRollingAppender = root.getAppender(LOG4CPLUS_C_STR_TO_TSTRING(FileRolling));
            ...
            {
                log4cplus::SharedAppenderPtr fileAppender(new log4cplus::RollingFileAppender(LOG4CPLUS_C_STR_TO_TSTRING(filename), 5 * 1024 * 1024, 5));
                fileAppender->setName(LOG4CPLUS_C_STR_TO_TSTRING(FileRolling));
                fileAppender->setLayout(std::auto_ptr<log4cplus::Layout>(new log4cplus::PatternLayout(LOG4CPLUS_C_STR_TO_TSTRING(PatternLayout))));
...
public:
    ...
    {
        log4cplus::Logger current = log4cplus::Logger::getInstance(LOG4CPLUS_C_STR_TO_TSTRING(m_name));
    ...
    {
        auto current = log4cplus::Logger::getInstance(LOG4CPLUS_C_STR_TO_TSTRING(m_name));
