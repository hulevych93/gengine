...
#ifdef _WIN32
  setsockopt(m_socket.lowest_layer().native(), SOL_SOCKET, SO_RCVTIMEO,
             (const char*)&seconds, sizeof(seconds));
  setsockopt(m_socket.lowest_layer().native(), SOL_SOCKET, SO_SNDTIMEO,
             (const char*)&seconds, sizeof(seconds));
#else
  struct timeval tv;
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
  setsockopt(m_socket.lowest_layer().native(), SOL_SOCKET, SO_RCVTIMEO, &tv,
             sizeof(tv));
  setsockopt(m_socket.lowest_layer().native(), SOL_SOCKET, SO_SNDTIMEO, &tv,
             sizeof(tv));
#endif
}

void SslSocket::AsyncConnect(const std::string& address,
                             std::uint16_t port,
                             connect_cb cb) {
  ...
  if (!error) {
    ...
    boost::asio::async_connect(
        ...
        [self, this, cb](const boost::system::error_code& ec,
