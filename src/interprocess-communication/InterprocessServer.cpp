...
InterprocessServer::InterprocessServer(const ipc_connection& connection,
                                       std::uint32_t threadId) {
  boost::apply_visitor(ServerInitializer(m_engine, m_acceptor, threadId),
                       connection);
}

InterprocessServer::~InterprocessServer() = default;

void InterprocessServer::AddExecutor(
    const std::shared_ptr<InterfaceImpl>& object) {
...
void InterprocessServer::RemoveExecutor(
    const std::shared_ptr<InterfaceImpl>& object) {
  std::lock_guard<std::mutex> lock(m_mutex);
}
...
void InterprocessServer::Accept() {
  m_acceptor->AcceptConnection([this](bool success,
    ...
    if (success) {
      ...
      auto handler = [this, &agent = *agent, queued = true](
                         bool success, std::uint32_t bytesProcessed,
                         const bool needToQueue) mutable {
        if (!success) {
      ...
      m_engine->RegisterConnection(agent->GetChannel(), std::move(handler));
