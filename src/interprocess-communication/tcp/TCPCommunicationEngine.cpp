...
TCPCommunicationEngine::TCPCommunicationEngine(std::uint32_t threadId)
    : Worker(threadId), m_engineImpl(std::make_unique<EngineImpl>()) {}

TCPCommunicationEngine::~TCPCommunicationEngine() = default;

void TCPCommunicationEngine::RegisterConnection(const IChannel& connection,
...
void TCPCommunicationEngine::StartInternal() {
  m_engineImpl->work =
      std::make_unique<boost::asio::io_context::work>(m_engineImpl->service);
