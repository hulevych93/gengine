...
PipeConnection GeneratePipeConnection(const std::string& key)
{
    ...
    static int count = 0;
    connection.pipe = std::wstring(L"") + utf8toWchar(key) + L"_" + std::to_wstring(++count) + L"_generated";
    return connection;
}
}

using TFactory = AbstractFactory<InterprocessCommunication::InterfaceImpl, std::string, const interface_key&, IMicroService&>;

class ExecutorBroker : public IExecutorBroker,
                       public Worker
{
public:
    ExecutorBroker();
    ...
    void Configure(const std::unordered_map<std::string, ServiceConfig>& config) override;
...
private:
    struct context
    {
        context(ExecutorBroker& parent, const std::string& id, const std::string& key)
            : id(id)
            , key(key)
            , parent(parent)
        {}
    ...
    {
        class ClientCreator : public IAbstractCreator<IMicroService, const interface_key&>
        {
        public:
            ClientCreator(IMicroService& handler)
                : handler(handler)
            {}
            std::shared_ptr<IMicroService> Create(const interface_key& key) const override
            {
        ...
        localContext(ExecutorBroker& parent, const std::string& id, const std::string& key)
        ...
        bool Run(const std::string& id, IMicroService& handler)
        {
            if (!registrator)
            {
                registrator = std::make_unique<StaticServiceRegistrator>(key, std::make_shared<ClientCreator>(handler), ServiceType::Local);
            }
            registrator->Do();
            return true;
        }
    ...
    {
        dynamicInterprocessContext(ExecutorBroker& parent, const std::string& id, const std::string& key, std::uint32_t threadId)
            : localContext(parent, id, key)
            , threadId(threadId)
        {}
            ...
            if (executor)
            {
                if (!server)
                {
                    ...
                    if (boost::apply_visitor(ConnectionValidator(), connection))
                    {
                        ...
                        auto iter = std::find_if(parent.signals.begin(), parent.signals.end(), [&existingServer, connection, this](const std::map<std::string, std::unique_ptr<context>>::value_type& other) {
                            auto ctx = dynamic_cast<dynamicInterprocessContext*>(other.second.get());
                            if (ctx)
                            {
                                auto result = connection == ctx->GetConnection() && ctx->server;
                                if (result)
                                {
                        ...
                        {
                            server = std::make_shared<InterprocessServer>(connection, threadId);
                        }
                    }
                }
        ...
        void Stop() override
        {
            if (server)
            {
                auto thatIter = std::find_if(parent.signals.begin(), parent.signals.end(), [this](const std::map<std::string, std::unique_ptr<context>>::value_type& other) {
                    auto ctx = dynamic_cast<dynamicInterprocessContext*>(other.second.get());
                    if (ctx)
                    {
                        return this != ctx && GetConnection() == ctx->GetConnection() && ctx->server;
                    }
                    return false;
                });
    ...
    {
        interprocessContext(ExecutorBroker& parent, const std::string& id, const std::string& key, const ipc_connection& data, std::uint32_t threadId)
            : dynamicInterprocessContext(parent, id, key, threadId)
            , data(data)
        {}

        ipc_connection GetConnection() override
        {
            return data;
        }
...
void ExecutorBroker::Configure(const std::unordered_map<std::string, ServiceConfig>& config)
{
    struct ServiceContextCreator : public boost::static_visitor<std::unique_ptr<context>>
    {
        ...
        {
            return std::make_unique<dynamicInterprocessContext>(parent, config.id, config.key, config.threadRef.get());
        }
        std::unique_ptr<context> operator()(const SharedConnection& connection) const
        {
            return nullptr;
        }
        std::unique_ptr<context> operator()(const InterprocessCommunication::ipc_connection& connection) const
        {
            return std::make_unique<interprocessContext>(parent, config.id, config.key, connection, config.threadRef.get());
        }
    ...
    {
        auto context = boost::apply_visitor(ServiceContextCreator(*this, service.second), service.second.connection);
        if (context)
        {
...
bool ExecutorBroker::Run(const std::string& id, IMicroService& handler)
{
    ...
    if (iter != signals.end())
    {
        ...
        auto task = [&context, id, &handler] { 
            return context->Run(id, handler); 
        };
