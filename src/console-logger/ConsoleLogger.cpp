...
class LoggerHandler : public ILogger {
 public:
  bool Init(const std::wstring& alias, bool console) override { return true; }
  bool LogTrace(const std::wstring& message) override { return true; }
...
class ConsoleLoggerModule : public Gengine::Entries::SlaveEntry {
 public:
  ConsoleLoggerModule(std::unique_ptr<IEntryToolsFactory>&& factory)
      : SlaveEntry(std::move(factory)), m_logger("CFC1399D") {}
