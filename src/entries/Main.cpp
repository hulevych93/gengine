...
class ConfigurationLock {
 public:
  ConfigurationLock(EntryConfig& config)
      : m_config(config), m_configured(false) {}
  ~ConfigurationLock() { Deconfigure(); }
  ...
  void Configure() {
    ...
    if (m_config.persistencyDir) {
      auto persistencyDir = Filesystem::CombinePath(
          toUtf8(Filesystem::GetAppFolder()), m_config.persistencyDir.get());
      GENGINE_INITIALIZE_PERSISTENCY(persistencyDir);
    }
    if (m_config.pluginsDir) {
      auto pluginsDir = Filesystem::CombinePath(
          Filesystem::GetAppFolder(), utf8toWchar(m_config.pluginsDir.get()));
...
Main::Main() : m_factory(makeFactory()) {
  assert(!Instance);
  Instance = this;
...
std::int32_t Main::Run(args_type args) {
  ...
  if (m_entry) {
    ...
    if (config_) {
      ...
      configLock = std::make_unique<ConfigurationLock>(
          const_cast<EntryConfig&>(*config));
  ...
  bool success = true;
  if (boost::apply_visitor(EmptyVisitor(), args) ||
      !ProcessCmdLine(args, &success)) {
    if (executor) {
...
std::shared_ptr<IEntry> Main::GetEntry(args_type args) const {
  ...
  try {
    ...
    GLOG_ERROR_INTERNAL("Failed to parse command options for entry point: %s",
                        ex.what());
  }
...
std::unique_ptr<EntryConfig> GetOptionalConfig(args_type args) {
  ...
  try {
    ...
    auto optionsObject = option_description.add_options();
    bpo::variables_map vm;
    optionsObject("config",
                  boost::program_options::value<std::string>(&configBuffer),
                  "additional config");
    boost::apply_visitor(ParseVisitor(vm, option_description), args);
    bpo::notify(vm);
