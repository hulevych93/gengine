...
bool AddServiceFailureActions(const std::wstring& name)
{
    ...
    if (hSCM)
    {
        ...
        if (hService)
        {
            ...
            return ChangeServiceConfig2(hService, SERVICE_CONFIG_FAILURE_ACTIONS, &servFailActions) == TRUE;
        }
        else
        {
...
bool IsServiceRunning(const std::wstring& name)
{
    ...
    if (hSCM)
    {
        ...
        if (hService)
        {
            ...
            {
                if (ssStatus.dwCurrentState == SERVICE_START_PENDING || ssStatus.dwCurrentState == SERVICE_RUNNING || ssStatus.dwCurrentState == SERVICE_STOP_PENDING)
                {
                    return true;
                }
            }
...
bool InstallService(const std::wstring& name)
{
    ...
    {
        auto serviceExePath = std::wstring{L"\""} + (Filesystem::GetExecutableFilePath()) + L"\"";
        serviceExePath += L" --entry=default";
        auto hSCM = ::OpenSCManager(nullptr, nullptr, SC_MANAGER_ALL_ACCESS);
        if (hSCM)
        {
            auto hService = ::CreateServiceW(
                hSCM, name.c_str(), name.c_str(),
                SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS | SERVICE_INTERACTIVE_PROCESS,
                SERVICE_AUTO_START, SERVICE_ERROR_NORMAL,
                serviceExePath.c_str(), nullptr, nullptr, L"RPCSS\0", nullptr, nullptr);
            if (hService)
            {
    ...
    if (IsServiceInstalled(name))
    {
        ...
        else
        {
            ...
            if (!((dwError == ERROR_SERVICE_NOT_ACTIVE) ||
                (dwError == ERROR_SERVICE_CANNOT_ACCEPT_CTRL && ssStatus.dwCurrentState == SERVICE_STOP_PENDING)))
            {
...
bool StopService(const std::wstring& name)
{
    if (IsServiceRunning(name))
    {
        ...
        if (hSCM != nullptr)
        {
            auto hService = ::OpenServiceW(hSCM, name.c_str(), SERVICE_STOP | SERVICE_QUERY_STATUS);
            if (hService != nullptr)
            {
                SERVICE_STATUS ssStatus;
                if (ControlService(hService, SERVICE_CONTROL_STOP, &ssStatus) == TRUE || (GetLastError() == ERROR_SERVICE_CANNOT_ACCEPT_CTRL && ssStatus.dwCurrentState == SERVICE_STOP_PENDING))
                {
                    Sleep(1000);
        ...
        if (hSCM != nullptr)
        {
            ...
            if (hService != nullptr)
            {
                ...
                if (ControlService(hService, SERVICE_CONTROL_STOP, &ssStatus) == TRUE ||
                    (GetLastError() == ERROR_SERVICE_CANNOT_ACCEPT_CTRL && ssStatus.dwCurrentState == SERVICE_STOP_PENDING))
                {
                    Sleep(1000);
...
std::wstring GetServiceExePath(const std::wstring& name)
{
    ...
    if (scm != nullptr)
    {
        ...
        if (service != nullptr)
        {
            ...
            {
                bufSize = bytesNeeded;
                serviceConfig = reinterpret_cast<LPQUERY_SERVICE_CONFIG>(::LocalAlloc(LMEM_FIXED, bufSize));
                if (QueryServiceConfig(service, serviceConfig, bufSize, &bytesNeeded) == TRUE)
                {
                    auto serviceExePath = EraseQuotesFromPath(serviceConfig->lpBinaryPathName);
