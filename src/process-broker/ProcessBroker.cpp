...
class ProcessLauncher : public Runnable {
 public:
  ProcessLauncher(IExecutableLauncherListener& listener,
                  const std::shared_ptr<ExecutableLauncher>& launcher,
                  const executable_params& params)
      : m_listener(listener), m_launcher(launcher), m_params(params) {}
 ...
 protected:
  ...
  void StopInternal() override { m_launcher->RemoveExecutable(m_params); }
...
class ProcessBroker : public IProcessBroker, public Worker {
 public:
  ProcessBroker() : Worker(11) {
    {
      ThreadConfig config;
      config.id = 11;
    ...
    auto task = [this, config]() {
      ...
      for (const auto& procConfig : config) {
        ...
        if (!procConfig.name.empty()) {
          ...
          processContext->launcher = std::make_unique<ProcessLauncher>(
              *processContext, m_launcher, params);
        } else if (!procConfig.service.empty()) {
#if _WIN32
          processContext->launcher = std::make_unique<ServiceLauncher>(
              utf8toWchar(procConfig.service));
#endif
        }
        if (!procConfig.lifetimeRef.empty()) {
          processContext->lifetime =
              std::make_unique<ServiceClientProxy<ILifetime>>(
                  procConfig.lifetimeRef);
        }
        signals.emplace(
            std::make_pair(procConfig.id, std::move(processContext)));
      }
    };
  ...
  connection PowerUp(uint32_t id, IProcessClient& client) override {
    ...
    auto task = [this, &conns, id, &client]() {
      ...
      if (iter != signals.end()) {
        ...
        conns.emplace_back(context->launched.connect(
            [&client] { client.OnProcessLauched(); }));
        conns.emplace_back(
            context->stopped.connect([&client] { client.OnProcessStopped(); }));
          ...
          if (context->counter == 0 || force) {
            if (context->lifetime) {
              ...
              try {
                ...
                GLOG_WARNING_INTERNAL(
                    "(*context->lifetime)->Shutdown(&success) call failed: %s",
                    ex.what());
              }
            }
