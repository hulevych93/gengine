    ...
    , m_serviceFileMappingData(nullptr)
{
    ...
    ::SetSecurityDescriptorDacl(securityAttributes.lpSecurityDescriptor, true, nullptr, false);

    m_serviceFileMappingHandle = ::CreateFileMappingW(INVALID_HANDLE_VALUE, &securityAttributes, PAGE_READWRITE, 0, 1024, mappingFileName);
    if (m_serviceFileMappingHandle)
    {
        m_serviceFileMappingData = ::MapViewOfFile(m_serviceFileMappingHandle, FILE_MAP_WRITE, 0, 0, sizeof(DWORD));
    ...
    else
    {
        ...
        {
            GLOG_ERROR("Failed to create file mapping: %ls; already exists", mappingFileName);
        }
        else
        {
            GLOG_ERROR("Failed to create file mapping: %ls; error: %u", mappingFileName, error);
        }
    }
}
...
{
    return m_serviceFileMappingData != nullptr && m_serviceFileMappingHandle != nullptr;
}
}
}