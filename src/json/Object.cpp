...
Object::Object(const storage_type& values) {
  ...
  std::copy(values.begin(), values.end(),
            std::back_inserter<storage_type>(m_values));
}

Object::Object(const Object& that) : Object(that.m_values) {}

Object::Object(Object&& that) : m_values(std::move(that.m_values)) {}
...
Object& Object::operator=(const Object& that) {
  if (this != &that) {
    m_values.clear();
    std::copy(that.m_values.begin(), that.m_values.end(),
              std::back_inserter<storage_type>(m_values));
  }
  return *this;
}
...
Object::iterator Object::find_by_key(const key_t& key) {
  return std::find_if(
      ...
      [&key](const std::pair<key_t, Value>& p) { return p.first == key; });
}
