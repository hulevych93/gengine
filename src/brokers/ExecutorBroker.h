...
using TExecutorCreator = std::shared_ptr<IAbstractCreator<InterprocessCommunication::InterfaceImpl, const InterprocessCommunication::interface_key&, IMicroService&>>;

class IExecutorBroker
{
public:
    virtual ~IExecutorBroker() = default;
    virtual void Configure(const std::unordered_map<std::string, ServiceConfig>& config) = 0;
