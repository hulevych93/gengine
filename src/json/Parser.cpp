...
void JSON::Parser::ParseValue(Location& start, Location& end, JSON::Value& root)
{
    m_position = start;
    m_start = start;
    m_end = end;
...
void JSON::Parser::CreateToken(Token& token, Token::Kind kind, Location& start)
{
    token.kind = kind;
    token.start = start;
}
    ...
    try
    {
        ...
        else
        {
            ...
            if (minus_sign)
            {
                ...
                if (uintValue < static_cast<JSON::uint64_t>(std::numeric_limits<JSON::int64_t>::max()))
                {
                    intValue -= uintValue;
...
{
    return (character == ',') || (character == '\0') || (character == ']') || (character == '}') || (character == '\r') || (character == ' ');
}
