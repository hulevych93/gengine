...
struct NamedPipeAcceptor::ListeningImpl {
  ListeningImpl() : handle(InvalidHandle) {}
...
void NamedPipeAcceptor::AcceptConnection(connected_callback callback) {
  if (CreateListeningHandle()) {
    ...
    m_engine->RegisterConnection(*m_listeningImpl->channel.get(), [this] {
      ...
      DWORD dwUnused = 0;
      OVERLAPPED ovr = {0};
      if (::GetOverlappedResult(m_listeningImpl->handle, &ovr, &dwUnused,
                                FALSE) == TRUE) {
        (*m_listeningImpl)();
      } else {
...
bool NamedPipeAcceptor::CreateListeningHandle() {
  if (m_listeningImpl->handle == InvalidHandle) {
    ...
    SetSecurityDescriptorDacl(securityAttributes.lpSecurityDescriptor, true,
                              NULL, false);
    m_listeningImpl->handle = CreateNamedPipeW(
        m_pipe.c_str(), PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
        PIPE_UNLIMITED_INSTANCES, 4 * 1024, 4 * 1024, 0, &securityAttributes);
