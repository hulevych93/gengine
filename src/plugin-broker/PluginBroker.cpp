...
class PluginBroker : public IPluginBroker, public Persistable<ISerializable> {
 public:
  PluginBroker() : Persistable("PluginBroker") {}
      ...
      if (plugin->Handshake(&info)) {
        ...
        if (config_) {
          ...
          if (!config->inServices.empty() || !config->outServices.empty()) {
            GENGINE_INITIALIZE_SERVICES(config->inServices,
                                        config->outServices);
          }
