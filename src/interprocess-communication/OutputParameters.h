...
private:
    ...
    void AppendContainer(const T& container, ParametersTypes containerType)
    {
        ...
        auto data = reinterpret_cast<std::uint8_t*>(AppendSizedParameter(blob->GetSize(), containerType));
...
private:
    ...
    void AppendSizedParameter(const void* data, std::uint32_t size, ParametersTypes type);
