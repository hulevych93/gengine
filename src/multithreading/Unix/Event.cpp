...
Event::Event() : m_bManualReset(false), m_bSignaled(false) {}
...
int Event::WaitMultiple(Event** ppEvents,
                        int iEventsCount,
                        bool bWaitAll,
  ...
  if (dwMilliseconds != Event::WAIT_INFINITE) {
    ...
    std::uint32_t uSec = (dwMilliseconds % 1000) *
                         1000;  // numer of microseconds besides seconds
    uSec += now_.tv_usec;
  ...
  if (bWaitAll) {
    ...
    for (i = 0; i < iEventsCount; ++i) {
      ...
      while (!event->m_bSignaled) {
        if (dwMilliseconds != Event::WAIT_INFINITE) {
          int iResult =
              pthread_cond_timedwait(&m->m_Cond, &m->m_Mutex, &to_time);
      ...
      if (Result == false) {  // timeout occured
        break;
      }
    }
    /*
    ...
    // one common MutexCond object is added to all event, to which we'll try
    // wait
    ...
    for (i = 0; i < iEventsCount; ++i) {
      Event* event = ppEvents[i];
      if (event->m_bSignaled) {  // success!
        Result = true;
        iEvent = i;
        break;
      }
    }
    if (Result == false) {  // there are no signaled event. wait some time...
      if (dwMilliseconds != Event::WAIT_INFINITE) {
      ...
      } else {  // infinite wauit for some event
      ...
      if (Result) {
        ...
        assert(iEvent >=
               0);  // if none of the events are signaled, why we're awaked?
      }
    }
