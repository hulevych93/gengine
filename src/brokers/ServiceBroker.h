...
class IServiceBroker {
 public:
  ...
  virtual void Configure(
      ...
      const std::unordered_map<std::string, ServiceConfig>& outServices) = 0;
  virtual void Deconfigure() = 0;
  virtual void Register(const std::string& id,
                        TServiceCreator&& creator,
                        ServiceType preffered) = 0;
...
class ServiceClientStub final : public ServiceClient, public Worker {
 public:
...
class ServiceClientProxy final : public ServiceClient {
 public:
  ServiceClientProxy(const std::string& key) : m_key(key) {}

  ServiceType* operator->() {
    InitService();
    if (!m_service)
      throw std::runtime_error(
          (std::string("bad service call ") + m_key.c_str()).c_str());
  ...
  operator bool() const { return m_service != nullptr; }
 ...
 protected:
  void InitService() {
    ...
    if (!m_service) {
      Connect(m_key);
      m_serviceCondition.wait_for(locker, std::chrono::seconds(30),
                                  [this]() { return m_service != nullptr; });
    }
  }
...
class StaticServiceRegistrator {
 public:
  StaticServiceRegistrator(const std::string& key,
                           TServiceCreator&& creator,
                           ServiceType preffered = ServiceType::Local);
...
#define GENGINE_REGISTER_SERVICE(key, handler, name, serviceType)              \
  static FactoryItem<StaticServiceRegistrator> const registrationBroker##name( \
      StaticServiceRegistrator(key, handler, serviceType));

#define GENGINE_ENABLE_SERVICES                \
  { Runtime<StaticServiceRegistrator>::Do(); } \
  while (false)                                \
    ;
#define GENGINE_DISABLE_SERVICES                 \
  { Runtime<StaticServiceRegistrator>::Undo(); } \
  while (false)                                  \
    ;

#define GENGINE_INITIALIZE_SERVICES(inServices, outServices) \
  GENGINE_ENABLE_SERVICES InitializeServices(inServices, outServices)
#define GENGINE_UNINITIALIZE_SERVICES \
