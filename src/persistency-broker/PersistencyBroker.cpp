...
bool writeBlob(const std::wstring& file, const std::shared_ptr<Blob>& blob)
{
    ...
    fileData.write(reinterpret_cast<const char*>(blob->GetData()), blob->GetSize());
    fileData.close();

    return true;
}

using persistency_read_signal = boost::signals2::signal<void(const IPersistency& persistency)>;
using persistency_write_signal = boost::signals2::signal<void(IPersistency& persistency)>;
...
public:
    void Configure(const std::string& directory) override
    {
        ...
        try
        {
            auto persistencyFiles = FileSeacher::Search(directory, L".*\\.persistency");
            for (const auto& persistencyFile : persistencyFiles)
            {
                ...
                signals.emplace(std::make_pair(toUtf8(persistencyFile->FileNameWithoutExtension()), std::move(persistency)));
            }
        }
    ...
    connection Subscribe(const std::string& id, IPersistencyClient& client) override
    {
        connection conn;
...
private:
    ...
    public:
        ...
        {
            connection conns;
            conns.emplace_back(read_signal.connect([&client](const IPersistency& persistency) { client.OnPersistency(persistency); }));
            conns.emplace_back(write_signal.connect([&client](IPersistency& persistency) { const_cast<const IPersistencyClient&>(client).OnPersistency(persistency); }));
            return conns;
        }
        ...
        void operator()(JSON::IJsonSerializable& object) const override
        {
            ...
            {
                JSON::Value root;
                std::string str(reinterpret_cast<char*>(data->GetData()), data->GetSize());
