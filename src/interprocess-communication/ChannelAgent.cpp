...
struct ChannelAgent::BufferImpl
{
    ...
    bool isReady() const
    {
        return requestHeader_->requestDataSize <= 0;
    }
...
{
    QueueVisitor(ChannelAgent& agent)
        : agent(agent)
    {}
...
ChannelAgent::ChannelAgent(std::unique_ptr<IChannel>&& impl, InterprocessServer& server)
...
bool ChannelAgent::ListeningChannelState::HandleIO(ChannelAgent& agent, std::uint32_t bytesProcessed)
{
...
{
    auto left = agent.m_buffer->requestHeader_->requestDataSize - agent.m_buffer->processed;
    assert(left > 0);
    auto buf = agent.m_buffer->data();
    return agent.m_impl->RecvAsync(buf + agent.m_buffer->processed, left);
}

bool ChannelAgent::ProcessingChannelState::HandleIO(ChannelAgent& agent, std::uint32_t bytesProcessed)
{
    auto left = agent.m_buffer->requestHeader_->requestDataSize - agent.m_buffer->processed;
    assert(left > 0);
    agent.m_buffer->processed += bytesProcessed;
...
{
    auto responseHeader = reinterpret_cast<ResponseHeader*>(agent.m_buffer->data());
    auto left = sizeof(ResponseHeader) + responseHeader->responseDataSize - agent.m_buffer->processed;
    assert(left > 0);
    auto buf = agent.m_buffer->data();
    return agent.m_impl->SendAsync(buf + agent.m_buffer->processed, left);
}

bool ChannelAgent::RespondingChannelState::HandleIO(ChannelAgent& agent, std::uint32_t bytesProcessed)
{
    auto responseHeader = reinterpret_cast<ResponseHeader*>(agent.m_buffer->data());
    auto left = sizeof(ResponseHeader) + responseHeader->responseDataSize - agent.m_buffer->processed;
    assert(left > 0);
...
void ChannelAgent::Execute()
{
    ...
    {
        if (!inputs->Deserialize(m_buffer->data(), m_buffer->requestHeader_->requestDataSize))
        {
    ...
    if (m_buffer->requestHeader_->request)
    {
        ...
        auto result = m_server.ProcessRequest(m_buffer->requestHeader_->functionCode,
    ...
    const std::shared_ptr<OutputParameters>& parameters)
{
    ...
    if (size > 0u)
    {
        memcpy(m_buffer->data() + sizeof(ResponseHeader), parameters->GetData(), size);
    }
}
