...
StaticServiceRegistrator::StaticServiceRegistrator(const std::string& key,
                                                   TServiceCreator&& creator,
                                                   ServiceType preffered)
    : m_key(key), m_creator(std::move(creator)), m_preffered(preffered) {}
...
ServiceClientStub::ServiceClientStub(const std::string& key)
    : Worker(222), m_key(key) {}

void ServiceClientStub::Execute(TServiceCall call) {
  if (InitService()) {
    auto handler = [this, call, service = m_service]() { call(service); };
...
void ServiceClientStub::Run() {
  if (m_service) {
    for (auto& call : m_calls) {
      auto handler = [this, call]() { call(m_service); };
      GENGINE_POST_TASK(handler);
    }
