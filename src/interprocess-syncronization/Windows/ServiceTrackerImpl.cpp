...
ServiceTrackerImpl::ServiceTrackerImpl(const std::wstring& mappingFileName, ServiceTracker::terminate_handler handler)
...
{
    m_fileMappingData = MapViewOfFile(m_fileMappingHandle, FILE_MAP_READ, 0, 0, sizeof(DWORD));
    m_thread = std::make_unique<std::thread>(&ServiceTrackerImpl::TrackerThreadProc, this);
    Multithreading::ThreadUtils::SetThreadName(*m_thread, L"ServiceTrackerThread");
}
...
{
    m_fileMappingHandle = OpenFileMappingW(FILE_MAP_READ, FALSE, m_mappingFileName.c_str());
...
void ServiceTrackerImpl::TrackerThreadProc()
{
    ...
    auto waitStatus = ::WaitForMultipleObjects(changeHandles.size(), &changeHandles[0], FALSE, INFINITE);
    switch (waitStatus)
    {
    case WAIT_OBJECT_0:
    {
