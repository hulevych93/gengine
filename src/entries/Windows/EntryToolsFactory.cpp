...
namespace Gengine {
namespace Entries {
std::vector<std::unique_ptr<Diagnostic::IDumper>> EntryToolsFactory::CreateDumpers(const std::wstring& module)
{
    std::vector<std::unique_ptr<Diagnostic::IDumper>> dumpers;
    dumpers.emplace_back(std::make_unique<ProcessMemoryDumper>(toUtf8(module), MiniDumpNormal, false));
...
std::unique_ptr<InstanceRegistratorInterface> EntryToolsFactory::CreateInstanceRegistrator(const std::wstring& module, InstanceType type)
{
    ...
    case InstanceType::OnePerUserSession:
        //objectName = (boost::wformat(module) % ProcessHelper::GetSessionKey(ProcessHelper::GetCurrentProcessId())).str();
        break;
    }

    if (!objectName.empty())
    {
        registrator = std::make_unique<SingleInstanceRegistratorImpl>(std::move(objectName));
    }
