...
void ExecutableLauncher::CheckDeadApps() {
  ...
  for (const auto& appIter : m_executableInstances) {
    ...
    if (!app->IsAlive()) {
      GLOG_INFO("Application %ls %ls died, schedule for remove.",
                app->GetParams().path, app->GetParams().params);
  ...
  for (const auto& deadApp : deadApps) {
    ...
    m_executableInstances.erase(
        std::make_pair(deadApp->GetSessionKey(), deadApp->GetParams()));
  }
}
...
void ExecutableLauncher::OnExecutableLaunched(
    const std::shared_ptr<Executable>& app) {}

void ExecutableLauncher::OnExecutableClosed(
    const std::shared_ptr<Executable>& app) {}
