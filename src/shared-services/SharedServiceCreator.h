...
std::shared_ptr<ServiceInterface> to_std(
    const boost::shared_ptr<ServiceInterface>& p) {
...
template <class ServiceInterface>
std::shared_ptr<ServiceInterface> import_creation_method(
    const SharedConnection& data) {
  auto libPath =
      Filesystem::CombinePath(toUtf8(Filesystem::GetAppFolder()), data.path);
  ...
  auto plugin = creator();
  auto lib = boost::make_shared<boost::dll::shared_library>(
      libPath, boost::dll::load_mode::append_decorations);
  return std::shared_ptr<ServiceInterface>(
      plugin, [lib](ServiceInterface* p) { delete p; });
}
...
class SymbolClient : public Services::IMicroService,
 ...
 public:
  std::shared_ptr<Services::IMicroService> Connect(
      const SharedConnection& data) override {
    if (!m_interface) {
