...
bool EntryBase::Finalize()
{
    ...
    {
        auto terminationHandler = [this, watchdogTime = config->watchdogTime.get()]() {
...
bool MasterEntry::Initialize()
{
    ...
    {
        m_instanceRegistrator = m_factory->CreateInstanceRegistrator(utf8toWchar(config->singleObjectName.get()));
        initialized = m_instanceRegistrator->RegisterInstance();
    }
    if (initialized && config->aliveObjectName)
    {
        m_aliveObject = m_factory->CreateAliveObject(utf8toWchar(config->aliveObjectName.get()));
    }
    return initialized;
}
...
{
    SlaveEntryImpl(SlaveEntry& entry)
        : entry(entry)
        , running(true)
    {}
...
bool SlaveEntry::Initialize()
{
    ...
    if (initialized && config->singleObjectName)
    {
        m_instanceRegistrator = m_factory->CreateInstanceRegistrator(utf8toWchar(config->singleObjectName.get()), InstanceType::OnePerUserSession);
        initialized = m_instanceRegistrator->RegisterInstance();
    }
    if (initialized && config->aliveObjectName)
    {
    ...
    if (initialized && config->lifetimeServiceId)
    {
        ...
        m_impl->lifetime = std::make_unique<Services::ServiceObjectProxy<ILifetime&>>(config->lifetimeServiceId.get(), *m_impl.get());
