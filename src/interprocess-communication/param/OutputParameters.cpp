...
template <class Type>
void OutputParameters::AppendIntegerParameter(Type value,
...
void OutputParameters::Append(const ISerializable& type) {
  ...
  AppendSizedParameter(blob->GetData(), blob->GetSize(),
                       ParametersTypes::BinarySerializable);
}
...
void* OutputParameters::AppendSizedParameter(std::uint32_t size,
                                             ParametersTypes type) {
  void* data = nullptr;
...
void OutputParameters::AppendSizedParameter(const void* data,
                                            std::uint32_t size,
                                            ParametersTypes type) {
  std::uint8_t* dest = nullptr;
  ParameterHeader* pHeader = nullptr;
