...
ServiceExecutor::StatusContext::StatusContext()
    : handle(nullptr)
    , status(std::make_unique<SERVICE_STATUS>())
{
    status->dwServiceType = SERVICE_WIN32_OWN_PROCESS;
    status->dwCurrentState = SERVICE_STOPPED;
    status->dwControlsAccepted = SERVICE_ACCEPT_SHUTDOWN | SERVICE_ACCEPT_STOP | SERVICE_ACCEPT_POWEREVENT
...
void ServiceExecutor::StatusContext::Update(std::uint32_t code)
{
    ...
    if (handle)
    {
        ...
        ::SetServiceStatus(reinterpret_cast<SERVICE_STATUS_HANDLE>(handle), status.get());
    }
}
...
bool ServiceExecutor::Execute(void* args)
{
    ...
    {
        { const_cast<wchar_t*>(name.data()), _ServiceMain },
        { nullptr, nullptr }
    };
...
bool ServiceExecutor::CreateProcessors(std::vector<std::unique_ptr<ICmdProcessor>>* processors)
{
    assert(processors);
...
void __stdcall ServiceExecutor::_ServiceMain(unsigned long dwArgc, wchar_t* lpszArgv[])
{
...
void ServiceExecutor::ServiceMain(std::uint32_t argc, wchar_t** argv)
{
    std::wstring name;
    GetEntry().GetAppName(&name);
    m_status.handle = RegisterServiceCtrlHandlerExW(const_cast<wchar_t*>(name.data()), _HandlerEx, nullptr);
    if (m_status.handle)
    {
...
std::uint32_t ServiceExecutor::HandlerEx(std::uint32_t control, std::uint32_t eventType,
    void* eventData, void* context)
{
    ...
    case SERVICE_CONTROL_SESSIONCHANGE:
        OnSessionChange(eventType, reinterpret_cast<WTSSESSION_NOTIFICATION*>(eventData));
        break;
    }
...
void ServiceExecutor::OnSessionChange(std::uint32_t reason, WTSSESSION_NOTIFICATION* notification)
{
...
bool ServiceExecutor::SWindowsServiceInstallCommand::Process(void* args, bool* success)
{
    std::wstring name;
...
ServiceExecutor::SWindowsServiceUninstallCommand::SWindowsServiceUninstallCommand()
    : CmdProcessorBase(L"uninstall")
{}

bool ServiceExecutor::SWindowsServiceUninstallCommand::Process(void* args, bool* success)
{
    std::wstring name;
    m_entry->GetAppName(&name);
...
bool ServiceExecutor::SWindowsServiceStartCommand::Process(void* args, bool* success)
{
    std::wstring name;
    m_entry->GetAppName(&name);
...
bool ServiceExecutor::SWindowsServiceStopCommand::Process(void* args, bool* success)
{
    std::wstring name;
    m_entry->GetAppName(&name);
...
bool ServiceExecutor::SWindowsServiceDebugCommand::Process(void* args, bool* success)
{
    assert(args);
    assert(success);
