...
template <int I> struct ToStringChoice : ToStringChoice<I + 1> { };
template <> struct ToStringChoice<100> { };

template<class T>
auto ToStringHelper(const T& value, ToStringChoice<0>) -> decltype(value.ToString())
{
    return value.ToString();
}

template<class T>
auto ToStringHelper(const T& value, ToStringChoice<1>) -> decltype(std::string(value))
{
    return std::string(value);
}

template<class T>
auto ToStringHelper(const T& value, ToStringChoice<2>) -> decltype(std::to_string(value))
{
    return std::to_string(value);
}

template<class T>
auto ToStringHelper(const T& value, ToStringChoice<3>) -> decltype(Gengine::toUtf8(value))
{
    return Gengine::toUtf8(value);
}

template <class T>
std::string ToStringHelper(T value, ToStringChoice<99>, std::enable_if_t<std::is_enum<T>::value, void*> = 0)
{
...
template<class T, class A>
std::string ToStringHelper(const std::unordered_set<T, A>& value, ToStringChoice<9>)
{
    return ToStringSingleHelper(value);
}
...
template<class T, class V, class A>
std::string ToStringHelper(const std::unordered_map<T, V, A>& value, ToStringChoice<11>)
{
    return ToStringPairedHelper(value);
}
...
bool operator<(const std::unordered_set<T, V>& left, const std::unordered_set<T, V>& right)
{
    return false;
}

template<class T, class V>
bool operator<(const std::unordered_map<T, V>& left, const std::unordered_map<T, V>& right)
{
    return false;
}
