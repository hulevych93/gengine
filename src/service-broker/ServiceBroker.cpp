...
class ServiceBroker : public IServiceBroker,
 ...
 public:
  ...
  void Configure(
      const std::unordered_map<std::string, ServiceConfig>& inServices,
      const std::unordered_map<std::string, ServiceConfig>& outServices)
      override;
  void Deconfigure() override;
  void Register(const std::string& id,
                TServiceCreator&& creator,
                ServiceType preffered) override;
 ...
 private:
  struct ServiceContextBase : public Worker {
    ServiceContextBase() : Worker(2) {}
    virtual ~ServiceContextBase() = default;

    virtual connection Subscribe(IServiceClient& client) {
      connection conns;
      conns.emplace_back(connected.connect([&client](const TService& service) {
        client.OnServiceAvailable(service);
      }));
      conns.emplace_back(
          disconnected.connect([&client](const TService& service) {
            client.OnServiceUnavailable(service);
          }));
      return conns;
    }

    virtual bool IsConnected() const { return service != nullptr; }
  ...
  struct ServiceContext : ServiceContextBase, public IProcessClient {
    ...
    void OnProcessLauched() override { Connect(); }
    ...
    bool Connect() override {
      ...
      struct ServiceConnector : public boost::static_visitor<bool> {
        ServiceConnector(ServiceContext& context) : m_context(context) {}
        ...
        bool operator()(const SharedConnection& connection) const {
          auto sharedService = std::dynamic_pointer_cast<ISharedLibraryClient>(
              m_context.service);
          if (sharedService) {
        ...
        bool operator()(const ipc_connection& connection) const {
          auto interprocessService =
              std::dynamic_pointer_cast<InterprocessClientInterface>(
                  m_context.service);
  ...
  struct CompositeServiceContext : ServiceContextBase, public IServiceClient {
...
ServiceBroker::ServiceBroker() : Worker(2) {
  {
    ThreadConfig config;
    config.id = 2;
      ...
      if (serviceConfig.type == ServiceType::Composite) {
        ...
        for (const auto& ref : serviceConfig.serviceRefs) {
          ...
          if (serviceFound != signals.end()) {
            compositeContext->contexts.emplace_back(
                std::static_pointer_cast<ServiceContext>(serviceFound->second));
          }
        }
        signals.emplace(
            std::make_pair(serviceIter.first, std::move(compositeContext)));
      } else {
        auto serviceContext = std::make_shared<ServiceContext>();
        ...
        if (serviceConfig.processRef) {
          serviceContext->processHolder = std::make_unique<ProcessHolder>(
              serviceConfig.processRef.get(), serviceContext.get());
        }
        ...
        signals.emplace(
            std::make_pair(serviceIter.first, std::move(serviceContext)));
      }
    }
...
connection ServiceBroker::Subscribe(const std::string& id,
