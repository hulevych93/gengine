...
ThreadPoolManager::ThreadPoolManager(std::int32_t threads)
    ...
    , m_threadCount(threads == -1 ? std::thread::hardware_concurrency() : threads)
{}
...
void ThreadPoolManager::StartInternal()
{
    ...
    {
        auto heartbeatThread = std::make_shared<std::thread>(boost::bind(&boost::asio::io_service::run, m_ioService));
