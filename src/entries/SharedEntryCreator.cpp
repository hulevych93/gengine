...
std::shared_ptr<IEntry> SharedEntryCreator::Create(
    std::unique_ptr<IEntryToolsFactory>&& factory) const {
  auto creator = boost::dll::import_alias<boost::shared_ptr<IEntry>(
      std::unique_ptr<IEntryToolsFactory> && factory)>(
      Filesystem::CombinePath(Filesystem::GetAppFolder(), m_moduleName),
      "create_shared_entry", boost::dll::load_mode::append_decorations);
