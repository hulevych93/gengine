  ...
  if (Send(interfaceKey, functionNumber, true, arguments)) {
    ...
    if (m_impl->Recv(&response_header, sizeof(response_header))) {
      if (response_header.responseCode == ResponseCodes::Ok) {
        ...
        GLOG_ERROR("Response failed; Code %08X",
                   static_cast<std::uint32_t>(response_header.responseCode));
        return false;
      }
    } else {
...
bool InterprocessClient::SendEvent(const interface_key& interfaceKey,
                                   std::uint8_t functionNumber,
                                   const OutputParameters& arguments) {
  if (Send(interfaceKey, functionNumber, false, arguments)) {
    ...
    if (m_impl->Recv(&response_header, sizeof(response_header))) {
      if (response_header.responseCode == ResponseCodes::Ok &&
          response_header.responseDataSize == 0) {
        return true;
      } else {
        GLOG_ERROR("Response failed; Code %08X",
                   static_cast<std::uint32_t>(response_header.responseCode));
...
bool InterprocessClient::Send(const interface_key& interfaceKey,
                              std::uint8_t functionNumber,
                              bool request,
                              const OutputParameters& arguments) {
  if (m_impl && m_impl->IsConnected()) {
    ...
    std::copy(interfaceKey.begin(), interfaceKey.begin() + 8,
              request_header.interfaceKey);
