...
bool LinuxSingleInstanceRegistrator::RegisterInstance() {
  ...
  if (m_iSingleInstanceFile != -1)
    return true;  // if we've successfully created this file this means that no
                  // other instances running
  int iFile = open(toUtf8(GetObjectName()).c_str(), O_CREAT | O_WRONLY,
                   S_IRWXU | S_IRWXG | S_IRWXO);
  if (iFile == -1) {
    GLOG_ERROR("Failed open file; erro %d", errno);
    assert(0);
    return false;  // some error occured - treat is as another instance is
                   // running
  }
  ...
  lock.l_len = 1;
  lock.l_type = F_WRLCK;
  if (fcntl(iFile, F_SETLK, &lock) ==
      -1) {  // we cannot acuire the lock - another instance is runing
    close(iFile);
    return false;
  }
